#0.Archivo que almacena y controla los timers de los mobs especiales

#1.Scoreboard general
scoreboard objectives add mob_timer dummy

#2.Timers de los cegadores
scoreboard players set #mce1 mob_timer 1200
scoreboard players set #mce2 mob_timer 1200
scoreboard players set #mce3 mob_timer 2400

#3.Timers de los zombies
scoreboard players set #mzo1 mob_timer 100
scoreboard players set #mzo2 mob_timer 160
scoreboard players set #mzo3 mob_timer 100
scoreboard players set #mzo4 mob_timer 100
scoreboard players set #mzo5 mob_timer 200
scoreboard players set #mzo6 mob_timer 200
scoreboard players set #mzo7 mob_timer 300
scoreboard players set #mzo8 mob_timer 600
scoreboard players set #mzo9 mob_timer 400
scoreboard players set #mzo10 mob_timer 200






